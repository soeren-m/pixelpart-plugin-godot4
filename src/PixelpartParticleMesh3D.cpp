...
namespace godot {
PixelpartParticleMesh3D::PixelpartParticleMesh3D(PixelpartGraphicsResourceStore& resourceStore, const pixelpart::Effect& effect, const pixelpart::ParticleType& particleType) :
...
void PixelpartParticleMesh3D::add_particle_trails(Node3D* parentNode,
	...
	for(auto& entry : trails) {
		...
		trail.directionToEdge[0u] = pixelpart::vec3_t(-trail.direction[0u].y, trail.direction[0u].x, trail.direction[0u].z);
...
void PixelpartParticleMesh3D::add_particle_meshes(Node3D* parentNode,
	...
	for(uint32_t p = 0u; p < numParticles; p++) {
		...
		uint32_t packedCustomX =
			(static_cast<uint32_t>(static_cast<float>(particleRenderData->velocity[p].x) * 0xFFFF) << 16) |
			(static_cast<uint32_t>(static_cast<float>(particleRenderData->force[p].x) * 0xFFFF) & 0xFFFF);
		uint32_t packedCustomY =
			(static_cast<uint32_t>(static_cast<float>(particleRenderData->velocity[p].y) * 0xFFFF) << 16) |
			(static_cast<uint32_t>(static_cast<float>(particleRenderData->force[p].y) * 0xFFFF) & 0xFFFF);
		uint32_t packedCustomZ =
			(static_cast<uint32_t>(static_cast<float>(particleRenderData->velocity[p].z) * 0xFFFF) << 16) |
			(static_cast<uint32_t>(static_cast<float>(particleRenderData->force[p].z) * 0xFFFF) & 0xFFFF);
		float packedCustomW = static_cast<float>(static_cast<pixelpart::float_t>(particleRenderData->id[p]) + glm::fract(particleRenderData->life[p] / packFactor));

		std::memcpy(&instanceData[p * 20 + 16], &packedCustomX, sizeof(float));
		std::memcpy(&instanceData[p * 20 + 17], &packedCustomY, sizeof(float));
		std::memcpy(&instanceData[p * 20 + 18], &packedCustomZ, sizeof(float));
