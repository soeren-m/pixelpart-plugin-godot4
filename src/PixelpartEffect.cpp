...
PixelpartEffect::PixelpartEffect() {
	...
	particleEngine = pixelpart::ParticleEngine::createUnique<pixelpart::ParticleSolverCPU>(nullptr, particleCapacity, PixelpartSystem::get_instance()->get_thread_pool());
}
PixelpartEffect::~PixelpartEffect() {
...
void PixelpartEffect::set_input_bool(String name, bool value) {
	...
	if(inputIt == effect.inputs.end()) {
		UtilityFunctions::push_warning("Effect input not found", name);
		return;
	}
...
void PixelpartEffect::set_input_int(String name, int value) {
	...
	inputIt->second.value = pixelpart::VariantValue::Int(static_cast<int64_t>(value));
	pixelpart::refreshEffectProperties(effect);
}
...
int PixelpartEffect::get_input_int(String name) const {
	...
	return inputIt->second.value.toInt();
}
...
pixelpart::EffectInputSet::iterator PixelpartEffect::findInput(String name) {
...
pixelpart::EffectInputSet::const_iterator PixelpartEffect::findInput(String name) const {
	std::string inputName = std::string(name.utf8().get_data());
